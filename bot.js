...
const LINK4M_KEY = process.env.LINK4M_KEY || '698ecc57359ef273e479d96b';
const UPTOLINK_KEY = process.env.UPTOLINK_KEY || '3c317fd7948df64ff7925c395c1db4dc5bf92527';
const SITE_URL = process.env.SITE_URL || 'https://iumoney.netlify.app';

const BASE_URL = `https://api.jsonbin.io/v3/b/${BIN_ID}`;

const TASKS = {
  'l4m':   { max: 2,    reward: 300, name: 'Link4M' },
  'uto1':  { max: 3636, reward: 350, name: 'Uptolink 1 Step', type: 2 },
  'uto2':  { max: 3636, reward: 350, name: 'Uptolink 2 Step', type: 3 },
  'uto3':  { max: 3636, reward: 400, name: 'Uptolink 3 Step', type: 4 },
  'uto4':  { max: 3636, reward: 400, name: 'Uptolink 4 Step', type: 5 },
};
...
app.post('/api/create-link', async (req, res) => {
  try {
    ...
    let shortUrl = '';
    if (taskId === 'l4m') {
      const apiUrl = `https://link4m.co/api-shorten/v2?api=${LINK4M_KEY}&url=${encodeURIComponent(destUrl)}`;
      const r = await fetch(apiUrl);
      const d = await r.json();
      if (d.status !== 'success') return res.status(500).json({ error: 'link4m_failed', detail: d.message });
      shortUrl = d.shortenedUrl;
    } else {
      const apiUrl = `https://uptolink.vip/api?api=${UPTOLINK_KEY}&url=${encodeURIComponent(destUrl)}&type=${task.type}`;
      const r = await fetch(apiUrl);
      const d = await r.json();
      if (d.status !== 'success') return res.status(500).json({ error: 'uptolink_failed', detail: d.message });
      shortUrl = d.shortenedUrl;
    }
...
app.post('/api/claim/:token', async (req, res) => {
  try {
    ...
    const task = TASKS[tokenData.taskId];
...
app.post('/api/withdraw', async (req, res) => {
  try {
    ...
    if (!userId || !amount || amount < 50000) return res.status(400).json({ error: 'invalid_request' });
