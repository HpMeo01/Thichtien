...
console.log('DEBUG API_KEY JSON:', JSON.stringify(API_KEY));
...
async function getBin() {
  ...
  const r = await fetch(BASE_URL + '/latest', { headers: { 'X-Access-Key': cleanKey } });
  console.log('DEBUG getBin status:', r.status);
  const d = await r.json();
  console.log('DEBUG getBin response keys:', Object.keys(d));
  if (r.status !== 200) {
    console.log('DEBUG getBin full response:', JSON.stringify(d));
  }
  return d.record;
}
async function saveBin(data) {
  ...
  const r = await fetch(BASE_URL, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json', 'X-Access-Key': cleanKey },
